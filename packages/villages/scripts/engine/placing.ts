...
import { boxOfRecord, type BuildingRecord } from "../core/building";
 ...
 * or the material.
 */
export function plan(dim: Dimension, key: string, origin: Vector3, rotation: Rotation, table: Vector3, free = false, palette = "", owner = ""): Plan | { refused: string } {
  ...
  const record: BuildingRecord = { dimId: dim.id, x: origin.x, y: origin.y, z: origin.z, key, rotation, sx: size.x, sy: size.y, sz: size.z, phase: "building", done: 0, table: { x: table.x, y: table.y, z: table.z }, free, palette, owner };
