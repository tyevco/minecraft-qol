...
**Villages, built as concepts** (`tools/structures/furfolk.ts`): every
people below has its buildings, a square, streets, greens and terminators,
pools and worldgen, generated by the same source as the four's and grown
by the offline expander into `concepts/villages/<people>.json` for the
viewer (`concept · villages`, with the pieces and buildings under `concept
· <people> village pieces` and `concept · <people> buildings`). Three things
differ from the text that follows, because they are concepts:

- **Job posts are lodestones.** The post block's `villages:people` state
  stops at 3, so a furfolk post cannot be written yet. Every post in a
  furfolk piece is a `lodestone` instead, which a processor list turns into
  a post once the people is appended (measured for exactly this block in
  `docs/villages-jigsaw-results.md`). The buildings' own `post()` calls
  still say which job goes where; `stampPeople` does the swap.
- **The pieces and worldgen go to the probe pack**, under
  `packages/probe/structures/villages/<people>/` and
  `packages/probe/worldgen/`, with the same `villages:` identifiers the
  shipped four use, as the four's did before the villages pack existed. The
  probe pack never ships; moving a people into the villages pack is a
  path change in `tools/structures/generate.ts` once `concept` comes off.
  Whether each generates in a plain world, and in the right biome, is §7
  item 7: none has been placed on a server yet.
- **Nothing is buried.** The fox den and the rabbit warren are turf mounds
  (the tinker burrow's shape) rather than houses cut into a bank, so no
  `terrain_adaptation: bury` is needed and §7 item 6 can wait. The
  squirrels' canopy is a deck five blocks up on jungle logs (the reedfolk's
  stilts, generalised: `People.deck`), with the ground left as grass
  beneath; the catfolk paddock has hay, a trough and fences but no sheep,
  since a structure carries no entities.

Each people's palette and pieces, as built, are noted at the end of each
**Village** entry; where a section's text and `furfolk.ts` disagree, the
code is what was judged in the viewer.
...
- **Village:** low spruce cabins with mossy roofs, half sunk into the ground
  ...
  round door and a chimney). Built with podzol paths, a mossy cobblestone
  roof on the cabins (moss has no stairs), and a spruce **lookout** to end
  streets.
...
- **Village:** terracotta and cherry planks, wool awnings everywhere, roofs
  ...
  and wool shelves). Built with smooth stone paths, cherry-plank roofs
  (terracotta has no stairs) and a pink-and-white awning over the loom
  house door; the paddock has no sheep yet.
...
- **Village:** a **warren**: houses cut into a low bank behind round doors,
  ...
  farmer). Built with packed mud paths and turf-mound warrens with birch
  door frames.
...
- **Village:** big log lodges, beehives on posts, a honey-coloured look.
  ...
  beehives on fence posts, flowers, a worker's post and a chest). Built
  with mossy cobblestone paths and whole-log walls.
...
- **Village:** sandstone and terracotta, flat roofs to sit on at dusk,
  ...
  open hall of pillars under one wide awning). Built with a sandstone
  **well** on the square, cactus where other peoples have trees, a
  **watch tower** to end streets, and no shared larder, whose spruce roof
  read wrong in the desert.
...
- **Village:** houses under and inside the giant mushrooms, mycelium
  ...
  red mushroom with the hall inside its stem). Built with mud brick paths
  on mycelium, giant red mushrooms where other peoples have trees, and
  no shared larder.
...
- **Village:** platforms in the canopy joined by rope bridges, the
  ...
  and a **nest hall** (the core: a round platform with a leaf dome). Built
  as a deck five up on jungle logs with jungle-fence rails; every piece
  stands on its own posts and the ground under them stays grass.
...
- **Village:** a holt of driftwood and stone at the tide line, boats
  ...
  side). Built with gravel paths on sand, a shingle green with driftwood,
  and driftwood where other peoples have trees.
...
- **Village:** a clearing people: few walls, many trees. Oak and birch,
  ...
  **glade hall** (the core: a ring of oak pillars under a leaf roof, open
  at the sides). Built with moss-block paths; the hall's roof is a leaf
  canopy over an oak-plank ceiling.
...
7. **The biome tags** for `taiga`, `cold`, `cherry_grove`, `flower_forest`,
   ...
   pack on the plain-world server as the tallfolk's did (`/place structure
   villages:<people>_village`, then new chunks in the biome), including the
   squirrels' deck on uneven jungle ground with no terrain adaptation and
   the otters' slipway at the water's edge. The ten structure sets have
   their own salts (a unit test says so), so none shares a cell with
   another's.
