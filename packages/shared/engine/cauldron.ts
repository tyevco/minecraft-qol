...
export function isCauldron(block: Block): boolean {
  try {
    return block.isValid && block.matches(CAULDRON);
  } catch {
    return false;
  }
}
...
export function readCauldron(block: Block): CauldronState | undefined {
  ...
  try {
    ...
    const level =
      (block.permutation.getState("fill_level") as number | undefined) ?? 0;
    ...
    const liquid =
      (block.permutation.getState("cauldron_liquid") as string | undefined) ??
      "water";
 ...
 */
export function planCauldronPermutation(
  next: CauldronState,
): BlockPermutation | undefined {
  const level = Math.max(0, Math.min(6, next.level));
  if (level === 0) {
    return BlockPermutation.resolve(CAULDRON, {
      fill_level: 0,
...
export function applyCauldron(
  ...
  effects?: readonly CauldronEffect[],
): void {
  if (permutation) {
    ...
    if (!fc)
      throw new Error(
        `cannot represent cauldron state ${next.fluid}/${next.level}`,
      );
    fc.fillLevel = next.level;
  }

  for (const effect of effects ?? []) {
    if (effect.kind === "add_dye") {
      const fc = block.getComponent(BlockComponentTypes.FluidContainer);
      const dyeType = ItemTypes.get(effect.dyeTypeId);
      if (fc && dyeType) fc.addDye(dyeType);
    }
  }
}

/** Plan and apply in one step, for callers with no compensation to arrange. */
export function writeCauldron(
  block: Block,
  next: CauldronState,
  effects?: readonly CauldronEffect[],
): void {
  applyCauldron(block, next, planCauldronPermutation(next), effects);
}

/**
 * Build the item a rule's input becomes.
 *
 * `transform` mutates the input stack so enchantments and durability survive;
 ...
 * Returns undefined when the rule produced no output (e.g. dye is consumed).
 */
export function buildOutput(
  ...
  if (output.mode === "new") {
    return new ItemStack(output.typeId, output.amount);
  }

  // transform: clear the dye in place
  const dyeable = input.getComponent(ItemComponentTypes.Dyeable);
