 ...
 * walk), the walk is vanilla pathing pointed at a beacon: a `builder:waypoint`
 * entity (nothing to draw, no gravity, gone in ninety seconds on its own) at
 * the spot, and the builder's `builder:walking` group, whose `follow_mob`
...
export const WAYPOINT = "builder:waypoint";
...
export function sendTo(person: Entity, spot: Vector3): void {
  ...
  try {
    ...
    person.triggerEvent("builder:walk");
  } catch {
...
export function halt(person: Entity | undefined): void {
  ...
  beacons.delete(person.id);
  try {
    if (person.isValid) person.triggerEvent("builder:halt");
  } catch {
    /* gone */
  }
}
