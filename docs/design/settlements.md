...
**Where the peoples live: five settlement shapes, twenty-five buildings, and the blueprint catalogue a builder works from**
...
- **One palette per people.** You should know whose settlement you are in
  ...
  gables on cobblestone footings (tallfolk), flat roofs behind false fronts
  on adobe and sun-bleached spruce (drover). Four blueprints per people,
  plus four shared ones any settlement can have.
...
### 2.6 Drover — the desert town

A street of false fronts on the sand. The trading post is the core: a
plank store with a porch, barrels behind the counter and a jukebox in the
corner, and the trader's post. Drovers live in adobe cabins, flat-roofed
with beam ends through the parapet and a porch to sit out the heat. The
corral is a work building: a fenced yard with a fodder strip of wheat
along one side, hay, and a shade roof over the water troughs, so its worker
is a farmer. The ranch is the other: a fenced pasture on grass with a pond
and a lean-to, whose worker becomes a rancher once the kids pen sheep in
it. The water tower is the watch: a plank tank on tall legs with a
railed platform, and the guard's post at its foot. On the square, a bell on
a post and a hitching rail with a trough.

```
   [cabin]  [cabin]  [corral]  [ranch]
        [ TRADING POST ]  [stall]
   [cabin]   [larder]   [water tower]
```

No trees: cacti stand on the square's corners and along the lanes, and
dead bushes where the other peoples have flowers.
...
Twenty-five blueprints, all generated by `tools/structures/buildings.ts`. Sizes
...
| Key | Building | Size | Blocks | Main materials | Role |
| --- | --- | --- | --- | --- | --- |
| `drover_trading_post` | Trading Post | 11×8×10 | 411 | spruce_planks 135, spruce_slab 112, smooth_sandstone 110 | core (trader) |
| `drover_cabin` | Adobe Cabin | 9×5×8 | 199 | hardened_clay 78, smooth_sandstone 56, spruce_slab 32 | home (builder) |
| `drover_corral` | Corral | 11×4×11 | 213 | sand 103, spruce_fence 47, farmland 16 | work (worker: farmer) |
| `drover_ranch` | Ranch | 13×4×11 | 217 | grass 137, spruce_fence 41, spruce_slab 16 | work (worker: rancher, once sheep are penned) |
| `drover_water_tower` | Water Tower | 7×15×7 | 280 | spruce_planks 119, spruce_stairs 48, stripped_spruce_log 42 | watch (guard) |
...
| wall | stone_bricks | mangrove_planks | bricks | oak_planks / spruce_planks | hardened_clay / spruce_planks | spruce_planks |
| corner | polished_deepslate | mangrove_log | copper_block | oak_log / spruce_log | stripped_spruce_log | spruce_log |
| roof | deepslate_tiles | bamboo_mosaic | cut_copper | dark_oak_planks | spruce_slab (flat) | spruce / dark_oak |
| ridge | polished_deepslate | mangrove_log | oxidized_copper | dark_oak_log | dark_oak_planks (false-front sign) | dark_oak_log |
| window | glass_pane | glass_pane | glass | glass_pane | glass_pane | glass_pane |
| awning | red / white wool | green / white wool | red / white wool | yellow / white wool | red / white wool | — |
