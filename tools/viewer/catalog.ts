/**
 * The model catalogue: every model the viewer shows, where it is on disk, and
 * where it sits in the sidebar. Pure data, so tools/viewer/tests can check it
 * without building anything; tools/viewer/build.ts copies what it names.
 *
 * The sidebar is three tabs (CATEGORIES), each grouped under headings: the
 * shipped blocks and entities by pack, with the concept entities together;
 * the concept buildings by people; the village pieces by people, with the
 * whole villages first. An entry says its `category` and `group`, or takes
 * the defaults: the models tab, headed by its pack.
 */

export type CategoryId = "models" | "buildings" | "villages";
...
export const CATEGORIES: Category[] = [
  { id: "models", title: "Models", blurb: "Blocks and entities, shipped and concept." },
  { id: "buildings", title: "Buildings", blurb: "Concept buildings, drawn in vanilla blocks." },
  { id: "villages", title: "Villages", blurb: "Whole villages, and the pieces they are assembled from." },
];
...
export interface Model {
  ...
  /** Sidebar tab; default "models". */
  category?: CategoryId;
  /** Heading the entry is listed under in its tab; default from the pack. */
  group?: string;
...
const AUTHORED: Model[] = [
  ...
  ...BUILDINGS.map(([id, name, people]): Model => ({
    ...
    kind: "structure",
    category: "buildings",
    group: people === "shared" ? "Shared" : peopleName(people),
  ...
  // The pieces a village is assembled from, under the people they belong to.
  ...VILLAGE_PIECES.map(([id, name, people]): Model => ({
    ...
    kind: "structure",
    category: "villages",
    group: peopleName(people),
    structure: `concepts/structures/${id}.json`,
  })),
];

/** A people's display name from its id (`wood_elf` is "Wood Elf"). */
function peopleName(id: string): string {
  return PEOPLES.find(([p]) => p === id)?.[1] ?? id.split("_").map((w) => w[0]!.toUpperCase() + w.slice(1)).join(" ");
}

const PACK_TITLES: Record<string, string> = {
  hearthstone: "Hearthstone", fluidworks: "Fluidworks", bulwark: "Bulwark", graves: "Graves", hatchling: "Hatchling", villages: "Villages",
};

/** An entry with its tab and heading filled in. */
export type Placed = Model & Required<Pick<Model, "category" | "group">>;

const placed: Placed[] = AUTHORED.map((m) => ({
  ...m,
  category: m.category ?? "models",
  group: m.group ?? (m.pack.startsWith("concept") ? "Concepts" : PACK_TITLES[m.pack] ?? m.pack),
}));

/**
 * The sidebar's order within a tab, over the catalogue's own: the shipped
 * packs before the concepts in the models tab, and the peoples in their
 * numbered order (PEOPLES) in the buildings tab as in the villages tab, with
 * the shared buildings last. The sort is stable, so entries keep their
 * catalogue order inside a group.
 */
function rank(m: Placed): number {
  if (m.category === "models") return m.group === "Concepts" ? 1 : 0;
  if (m.category === "buildings") {
    const i = PEOPLES.findIndex(([, name]) => name === m.group);
    return i < 0 ? PEOPLES.length : i;
  }
  return 0;
}

export const MODELS: Placed[] = placed.sort((a, b) => rank(a) - rank(b));
