...
export interface BuildingRecord extends Position {
  ...
  palette: string;
  /** The player who placed it (their id), or "" from the hatch: who earns the standing when it is finished. */
  owner: string;
}

export type Row = [string, number, number, number, string, number, number, number, number, number, number, number, number, number, number, string, string];

export function packRecord(r: BuildingRecord): Row {
  return [r.dimId, r.x, r.y, r.z, r.key, r.rotation, r.sx, r.sy, r.sz, PHASES.indexOf(r.phase), r.done, r.table.x, r.table.y, r.table.z, r.free ? 1 : 0, r.palette, r.owner];
}
...
export function unpackRecord(packed: unknown): BuildingRecord | undefined {
  ...
  // A schema-1 row has no `free` column: it was paid for. A schema-2 row has no palette: the blueprint's own. A schema-3 row has no owner.
  const [dimId, x, y, z, key, rotation, sx, sy, sz, phase, done, tx, ty, tz, free = 0, palette = "", owner = ""] = packed as unknown[];
  ...
  return { dimId, x, y, z, key, rotation: rotation as Rotation, sx, sy, sz, phase: PHASES[phase]!, done, table: { x: tx, y: ty, z: tz }, free: free === 1 || free === true, palette: typeof palette === "string" ? palette : "", owner: typeof owner === "string" ? owner : "" };
}
